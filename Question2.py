...
def query_string(user_string):
    ...
    # ps = per_search since BM25 _score changes depending on user_strdef query_string(user_string
    result_params_list_df_ps = pd.DataFrame(columns=['movieId', 'title', 'genres', 'BM25_score'])

    res = es.search(index="movies", body={"query": {"match": {"title":"{}".format(user_strdef query_string(user_string)}}}, size = 1000)
    hits = res['hits']['total']['value'] #total hits
...
def startLoop(movie_avg_rating_df, movie_rating_df_pu):
    ...
    user_strdef query_string(user_string_movie = input("Which movie do you want? (by title): \n")
    
    while( user_strdef query_string(user_string_movie != '//exit' ) :
        
        user_strdef query_string(user_string_user = input("For which user do you want to search? (int): \n")
        while ( ( user_strdef query_string(user_string_user.isdigit() == False) ):
            user_strdef query_string(user_string_user = input("For which user do you want to search? (int): \n")

        #get ranking from Elasticsearch
        query_result_params_df = qudef query_string(user_string_movie)
        #compute final ranking based on additional data ( movie_avg_rating_df, movie_rating_df_pu )
        final_df = finalRanking(query_result_params_df, movie_avg_rating_df, movie_rating_df_pu, user_strdef query_string(user_string_user)
        #print results
        print(final_df)
...
def finalRanking(query_result_params_df, movie_avg_rating_df, movie_rating_df_pu, user_strdef query_string(user_string_user):
    ...
    temp.drop(temp[temp['userId'] != int(user_strdef query_string(user_string_user)].index, inplace=True)
    #add user_rating
